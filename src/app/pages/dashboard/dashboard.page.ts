...
export class DashboardPage {
  ...
  private loadAll(force = false): Promise<void> {
    ...
    return new Promise((resolve) => {
      forkJoin({
        ...
        weekly: this.api
          .getWeeklyOverview(this.weekOffset)
          .pipe(catchError(() => of(null))),
      })
        .pipe(
          finalize(() => {
        ...
        .subscribe(async ({ appointments, stats, weekly }) => {
          ...
          const mapped = this.mapRows(this.extractList(appointments));
          ...
          if (!appointments)
            await this.toast("Failed to load today's appointments.");
        });
    });
  }
  ...
  private refreshLineChart(raw: any) {
    ...
    for (const item of list) {
      const iso = this.toISODate_LocalSafe(
        item?.date ?? item?.appointmentDate ?? item?.day,
      );
      const slot = days.find((d) => d.iso === iso);
      if (slot)
        slot.count = Number(item?.count ?? item?.total ?? item?.patients ?? 0);
    }
  ...
  private applyCardFilter(
    ...
    return list.filter(
      (r) =>
        Number(r.statusCode) === map[key as Exclude<CardFilterKey, 'today'>],
    );
  }

  private applySearchOnList(
    list: AppointmentRow[],
    qRaw: string,
  ): AppointmentRow[] {
  ...
  private buildCards(rows: AppointmentRow[]): DashboardCards {
    ...
    for (const r of rows) {
      ...
      else if (r.statusCode === AppointmentStatus.AwaitingPayment)
        c.awaitingPayment++;
  ...
  get resolvedRole(): string {
    return this.profileRole || localStorage.getItem('mhc_role') || 'Doctor';
  }
